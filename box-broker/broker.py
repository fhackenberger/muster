...
Config (env, from compose):
  ...
  VAULT_ENV_FILE      KEY=VALUE lines GENERATED by vault-sync.sh — this stack's agent-vault token and
                      the claude launcher; applied between the two above
...
# The agent-vault half: AGENT_VAULT_ADDR/VAULT/TOKEN and MUSTER_CLAUDE_LAUNCHER, written by
# vault-sync.sh. It is GENERATED rather than templated because the token does not exist until the
# vault has minted it — which is also why it is applied here and not folded into service-env, whose
# other consumer (the hub, via compose `env_file:`) would then need the file to exist before the
# first `up`. Optional: a stack without agent-vault has no such file and nothing changes.
...
def parse_env_lines(path, what):
	"""KEY=VALUE lines from an env file, as a list of "KEY=VALUE" strings. Missing file -> [].

	The format is compose's env_file format, which is what service-env has to be (compose itself
	feeds that one to the hub) and therefore what the files beside it are too — one KEY=VALUE per
	line, '#' comments, no quoting rules and no shell expansion, the value taken literally to the end
	of the line. Keys are validated; a malformed line is rejected loudly rather than silently shipping
	something odd into every box. Values cannot contain newlines (the list is newline-joined when
	handed to muster-box.sh).

	`what` names the file in the error, because by the time a bad line reaches a box the only clue is
	this message."""
	out = []
	...
	with open(path) as fh:
		for raw in fh:
			...
			if not line or line.startswith("#"):
				continue
			if "=" not in line:
				raise ValueError(f"bad {what} line {line!r} (want: KEY=VALUE)")
			key, value = line.split("=", 1)
			key = key.strip()
			if not ENV_KEY_RE.match(key):
				raise ValueError(f"bad {what} key {key!r}")
			out.append(f"{key}={value}")
	return out


def parse_service_env():
	"""The project's service settings — given to the hub by compose AND to every box by us, verbatim.

	One file, both sides, so a backend behaves the same whichever of them runs it. That is also why
	it must never contain `$VAR`: compose interpolates it against the host, we do not."""
	return parse_env_lines(SERVICE_ENV_FILE, "service-env")


def parse_vault_env():
	"""What vault-sync.sh generated: this stack's agent-vault address, vault name, token and launcher.

	Applied AFTER service-env and BEFORE box-env. After service-env because the token is the newer,
	more specific fact; before box-env because box-env is the project's last word on anything, and a
	project that wants one box outside the proxy should be able to say so by unsetting the launcher.

	Read at every spawn rather than cached: ./vault-sync.sh may rotate the token while this broker is
	running, and a value cached at import would leave every later box authenticating with a token the
	vault has already invalidated."""
	return parse_env_lines(VAULT_ENV_FILE, "vault-env")
...
def parse_box_env():
	...
	return parse_env_lines(BOX_ENV_FILE, "box-env")
...
def create_box(name, resume=False, fresh_upper=False, base=None, merge=None):
	...
	# The credential broker's half, when this stack has one. Appended here rather than at the end so
	# box-env still gets the last word, and so these values land in `facts` below and can be referred
	# to from box-env like any other ($AGENT_VAULT_ADDR in a project's own variable, say).
	svc_env.extend(parse_vault_env())
